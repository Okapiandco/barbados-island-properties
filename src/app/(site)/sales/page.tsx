import { client, salesPropertiesQuery, locationsQuery, collectionsQuery, amenitiesQuery } from '@/lib/sanity'
...
export const metadata = {
  title: 'Properties For Sale in Barbados | Barbados Island Properties',
  description: 'Browse luxury properties for sale in Barbados - villas, apartments, land and more.',
}
...
export default async function SalesPage({
  ...
  const [allProperties, locations, collections, amenities] = await Promise.all([
    client.fetch(salesPropertiesQuery),
  ...
  if (searchParams.minPrice) {
    ...
    properties = properties.filter((p: any) => p.priceUsd != null && p.priceUsd >= min)
  }
  if (searchParams.maxPrice) {
    const max = parseInt(searchParams.maxPrice)
    properties = properties.filter((p: any) => p.priceUsd != null && p.priceUsd <= max)
  }

  return (
    <div className="container-max py-10">
      <h1 className="text-3xl md:text-4xl font-heading mb-2">Properties For Sale in Barbados</h1>
        ...
        <div className="flex-1">
          {properties.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-400">Try adjusting your filters or check back soon for new listings</p>
            </div>
          )}
        </div>
      </div>
